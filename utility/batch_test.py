...
data_generator = Data(args,path=args.data_path + args.dataset, batch_size=args.batch_size, dataset = args.dataset)
# sys.exit()
...
def test_cpp(users_to_test, embedding_h, coldstart=False):
    ...
    if coldstart == True:
        test_set = data_generator.cst_test_set
        train_items = data_generator.cst_train_items
    else:
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        for user in user_batch:
            test_items.append(test_set[user])  # (B, #test_items)

        # set the ranking scores of training items to -inf,
        # then the training items will be sorted at the end of the ranking list.
        for idx, user in enumerate(user_batch):
            train_items_off = train_items[user]
