...
class MultiHeadGATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_edges, num_heads=1, merge='sum'):
        super(MultiHeadGATLayer, self).__init__()
        self.heads = nn.ModuleList()
        for i in range(num_heads):
            # print(in_dim, out_dim)
            self.heads.append(GATLayer(in_dim, out_dim, num_edges))
        self.merge = merge
...
class GATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_edges):
        ...
        self.bias = nn.Parameter(torch.randn(num_edges), requires_grad=True)
        # equation (2)
    ...
    def edge_attention_dot_product(self, edges):
        # edge UDF for equation (2)

        print(edges.src['z'].shape, edges.dst['z'].shape)
        z2 = torch.sum(torch.mul(edges.src['z'], edges.dst['z']) / (edges.src['z'].shape[1] ** (1 / 2)), dim=1)
        z2 = z2 + self.bias
        print('z2',z2.shape)
        # sys.exit()
        # a = self.attn_fc(z2)
    ...
    def reduce_func_dot_product(self, nodes):
        # reduce UDF for equation (3) & (4)
        # equation (3)
        print('mailbox_e',nodes.mailbox['e'].shape)
        alpha = F.softmax(nodes.mailbox['e'], dim=1)
        print('alpha',alpha.shape)
        # sys.exit()
        # equation (4)
        print('mailbox_z',nodes.mailbox['z'].shape)
    ...
    def forward(self, graph, h, etype):
        with graph.local_scope():
            ...
            # equation (2)
            graph.apply_edges(self.edge_attention_dot_product, etype=etype)
            # equation (3) & (4)
            graph.update_all(self.message_func, self.reduce_func_dot_product, etype=etype)
            # print(etype)
